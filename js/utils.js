...
// --- ESTILOS COMPARTIDOS ---
const dropdownClasses = "autocomplete-dropdown hidden absolute w-full max-h-48 overflow-y-auto bg-dark-800 border border-gray-700 rounded-lg shadow-xl z-50 mt-1";
const itemClasses = "autocomplete-item p-3 hover:bg-brand-600 hover:text-white cursor-pointer text-sm text-gray-300 border-b border-gray-700/50 last:border-0 transition-colors";
...
function renderInputTags() {
    ...
    currentTeams.forEach(team => {
        ...
        tag.className = "tag-element flex items-center gap-1 bg-brand-600/20 text-brand-200 border border-brand-500/30 px-2 py-1 rounded text-xs font-semibold mr-1 mb-1 animate-fadeIn";
        tag.innerHTML = `
...
function setupFormAutocomplete() {
    ...
    input.addEventListener("input", function() {
        ...
        const matches = getAllTeams().filter(t => 
            t.toLowerCase().includes(val) && !currentTeams.includes(t)
        );
